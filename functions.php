...
add_action('after_setup_theme', 'rdave_theme_support');


function rdave_menus(){

    $locations = array(
        "primary"=> "Destkop Primary Left Sidebar",
...
add_action('init', 'rdave_menus');



function rdave_register_styles(){
    ...
    wp_enqueue_style('followandrew-fontawesome', 'https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.13.0/css/all.min.css', array(), $version, 'all');
...
add_action('wp_enqueue_scripts', 'rdave_register_styles');


function rdave_register_scripts(){
...
add_action('wp_enqueue_scripts', 'rdave_register_scripts');


function rdave_widget_areas(){
    register_sidebar(
        array(
            'before_title'=> '<h2 style="color:white !important">',
            'after_title'=>'</h2>',
            'before_widget'=> '',
            'after_widget'=>'',
            'name'=>'Sidebar Area',
...
add_action('widgets_init', 'rdave_widget_areas');